...
import { getAiVectorizerParameters, listAiLlms } from "../graphql/queries";
import { updateAiVectorizerParameters } from "../graphql/mutations";
...
export default function AiVectorizerParametersUpdateForm(props) {
  const {
    id: idProp,
    aiVectorizerParameters: aiVectorizerParametersModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = aiVectorizerParametersRecord
      ? { ...initialValues, ...aiVectorizerParametersRecord, aiLlm }
      : initialValues;
    setName(cleanValues.name);
    setAiLlm(cleanValues.aiLlm);
    setCurrentAiLlmValue(undefined);
    setCurrentAiLlmDisplayValue("");
    setChunkSize(cleanValues.chunkSize);
    setChunkOverlap(cleanValues.chunkOverlap);
    setErrors({});
  };
  const [aiVectorizerParametersRecord, setAiVectorizerParametersRecord] =
    React.useState(aiVectorizerParametersModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getAiVectorizerParameters.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAiVectorizerParameters
        : aiVectorizerParametersModelProp;
      const aiLlmRecord = record ? await record.aiLlm : undefined;
      setAiLlm(aiLlmRecord);
      setAiVectorizerParametersRecord(record);
    };
    queryData();
  }, [idProp, aiVectorizerParametersModelProp]);
  React.useEffect(resetStateValues, [aiVectorizerParametersRecord, aiLlm]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            aiLlmID: modelFields?.aiLlm?.id ?? null,
            chunkSize: modelFields.chunkSize,
            chunkOverlap: modelFields.chunkOverlap,
          };
          await client.graphql({
            query: updateAiVectorizerParameters.replaceAll("__typename", ""),
            variables: {
              input: {
                id: aiVectorizerParametersRecord.id,
                ...modelFieldsToSave,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "AiVectorizerParametersUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || aiVectorizerParametersModelProp)}
