...
export default function BackendDatabaseUpdateForm(props) {
  const {
    id: idProp,
    backendDatabase: backendDatabaseModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = backendDatabaseRecord
      ? {
          ...initialValues,
          ...backendDatabaseRecord,
          type,
          usedIn: linkedUsedIn,
        }
      : initialValues;
  ...
  const [backendDatabaseRecord, setBackendDatabaseRecord] = React.useState(
    backendDatabaseModelProp
  );
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getBackendDatabase.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getBackendDatabase
        : backendDatabaseModelProp;
      const typeRecord = record ? await record.type : undefined;
      setType(typeRecord);
      const linkedUsedIn = record
        ? (
            await client.graphql({
              query:
                backendDatabaseBackendSettingsByBackendDatabaseId.replaceAll(
                  "__typename",
                  ""
                ),
              variables: {
                backendDatabaseId: record.id,
              },
            })
          ).data.backendDatabaseBackendSettingsByBackendDatabaseId.items.map(
            (t) => t.backendSettings
          )
        : [];
      setLinkedUsedIn(linkedUsedIn);
      setBackendDatabaseRecord(record);
    };
    queryData();
  }, [idProp, backendDatabaseModelProp]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
                ...
                variables: {
                  filter: {
                    and: [
                      ...
                      { backendDatabaseId: { eq: backendDatabaseRecord.id } },
                    ],
                  },
                },
              })
            ...
            for (let i = count; i > 0; i--) {
              promises.push(
                client.graphql({
                  ...
                  variables: {
                    input: {
                      backendDatabaseId: backendDatabaseRecord.id,
          ...
          const modelFieldsToSave = {
            ...
            backendDatabaseTypeID: modelFields?.type?.id ?? null,
            reference: modelFields.reference,
          };
          promises.push(
            client.graphql({
              query: updateBackendDatabase.replaceAll("__typename", ""),
              variables: {
                input: {
                  id: backendDatabaseRecord.id,
                  ...modelFieldsToSave,
                },
              },
            })
      ...
      {...getOverrideProps(overrides, "BackendDatabaseUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || backendDatabaseModelProp)}
