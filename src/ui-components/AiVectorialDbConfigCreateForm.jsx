...
import { createAiVectorialDbConfig } from "../graphql/mutations";
const client = generateClient();
export default function AiVectorialDbConfigCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setIndexName(initialValues.indexName);
    setNameSpace(initialValues.nameSpace);
    setScore(initialValues.score);
    setNumberOfSimilarities(initialValues.numberOfSimilarities);
    setErrors({});
  };
  const validations = {
    name: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createAiVectorialDbConfig.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "AiVectorialDbConfigCreateForm")}
      {...rest}
    >
      <TextField
        label="Name"
