...
export default function BackendEndpointCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setParametersWIP(initialValues.parametersWIP);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            methodID: modelFields?.method?.id,
            parametersWIP: modelFields.parametersWIP,
          };
          const backendEndpoint = (
            await client.graphql({
              query: createBackendEndpoint.replaceAll("__typename", ""),
              variables: {
                input: {
                  ...modelFieldsToSave,
                },
          ...
          )?.data?.createBackendEndpoint;
          const promises = [];
          promises.push(
            ...usedInSettings.reduce((promises, backendSettings) => {
              promises.push(
                client.graphql({
          ...
          promises.push(
            ...usedInTool.reduce((promises, aiApiTool) => {
              promises.push(
                client.graphql({
      ...
      {...getOverrideProps(overrides, "BackendEndpointCreateForm")}
      {...rest}
    >
      <TextField
        label="Name"
