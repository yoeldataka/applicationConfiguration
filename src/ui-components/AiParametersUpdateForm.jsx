...
import { updateAiParameters } from "../graphql/mutations";
...
export default function AiParametersUpdateForm(props) {
  const {
    id: idProp,
    aiParameters: aiParametersModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = aiParametersRecord
      ? { ...initialValues, ...aiParametersRecord, company, llm }
      : initialValues;
    ...
    setConversationPurpose(cleanValues.conversationPurpose);
    setConversationType(cleanValues.conversationType);
    setPrePrompt(cleanValues.prePrompt);
    setPostPrompt(cleanValues.postPrompt);
    setLlm(cleanValues.llm);
    setCurrentLlmValue(undefined);
    setCurrentLlmDisplayValue("");
    setChatMemory(cleanValues.chatMemory);
    setResponseTemperature(cleanValues.responseTemperature);
    setResponseLanguage(cleanValues.responseLanguage);
    setErrors({});
  };
  const [aiParametersRecord, setAiParametersRecord] = React.useState(
    aiParametersModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getAiParameters.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAiParameters
        : aiParametersModelProp;
      const companyRecord = record ? await record.company : undefined;
      setCompany(companyRecord);
      const llmRecord = record ? await record.llm : undefined;
      setLlm(llmRecord);
      setAiParametersRecord(record);
    };
    queryData();
  }, [idProp, aiParametersModelProp]);
  React.useEffect(resetStateValues, [aiParametersRecord, company, llm]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            companyID: modelFields?.company?.id ?? null,
          ...
          await client.graphql({
            query: updateAiParameters.replaceAll("__typename", ""),
            variables: {
              input: {
                id: aiParametersRecord.id,
                ...modelFieldsToSave,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "AiParametersUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || aiParametersModelProp)}
