...
import { listAiToolGroups, listFrontendComponents } from "../graphql/queries";
import {
  createAiRenderTool,
...
export default function AiRenderToolCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setFunc(initialValues.func);
    setUseCases(initialValues.useCases);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            payload: modelFields.payload,
            action: modelFields.action,
          };
          const aiRenderTool = (
            await client.graphql({
              query: createAiRenderTool.replaceAll("__typename", ""),
              variables: {
                input: {
                  ...modelFieldsToSave,
                },
          ...
          )?.data?.createAiRenderTool;
          const promises = [];
          promises.push(
            ...component.reduce((promises, frontendComponent) => {
              promises.push(
                client.graphql({
          ...
          promises.push(
            ...usedIn.reduce((promises, aiToolGroup) => {
              promises.push(
                client.graphql({
      ...
      {...getOverrideProps(overrides, "AiRenderToolCreateForm")}
      {...rest}
    >
      <TextField
        label="Name"
