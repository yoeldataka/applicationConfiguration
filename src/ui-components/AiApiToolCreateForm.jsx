...
import { listAiToolGroups, listBackendEndpoints } from "../graphql/queries";
import {
  createAiApiTool,
...
export default function AiApiToolCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const aiApiTool = (
            await client.graphql({
              query: createAiApiTool.replaceAll("__typename", ""),
              variables: {
                input: {
                  ...modelFieldsToSave,
                },
          ...
          )?.data?.createAiApiTool;
          const promises = [];
          promises.push(
            ...endpoint.reduce((promises, backendEndpoint) => {
              promises.push(
                client.graphql({
      ...
      {...getOverrideProps(overrides, "AiApiToolCreateForm")}
      {...rest}
    >
      <TextField
        label="Name"
