...
export default function BackendSettingsUpdateForm(props) {
  const {
    id: idProp,
    backendSettings: backendSettingsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = backendSettingsRecord
      ? {
          ...initialValues,
          ...backendSettingsRecord,
    ...
    setSecretName(cleanValues.secretName);
    setEndpoints(cleanValues.endpoints ?? []);
    setCurrentEndpointsValue(undefined);
    setCurrentEndpointsDisplayValue("");
    setDatabases(cleanValues.databases ?? []);
  ...
  const [backendSettingsRecord, setBackendSettingsRecord] = React.useState(
    backendSettingsModelProp
  );
  const [linkedEndpoints, setLinkedEndpoints] = React.useState([]);
  const canUnlinkEndpoints = false;
  const [linkedDatabases, setLinkedDatabases] = React.useState([]);
  const canUnlinkDatabases = false;
  const [linkedQueues, setLinkedQueues] = React.useState([]);
  const canUnlinkQueues = false;
  const [linkedTopics, setLinkedTopics] = React.useState([]);
  const canUnlinkTopics = false;
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getBackendSettings.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getBackendSettings
        : backendSettingsModelProp;
      const linkedEndpoints = record
        ? (
            await client.graphql({
              query:
                backendEndpointBackendSettingsByBackendSettingsId.replaceAll(
                  "__typename",
                  ""
                ),
              variables: {
                backendSettingsId: record.id,
              },
            })
          ).data.backendEndpointBackendSettingsByBackendSettingsId.items.map(
            (t) => t.backendEndpoint
          )
        : [];
      setLinkedEndpoints(linkedEndpoints);
      const linkedDatabases = record
        ? (
            await client.graphql({
              query:
                backendDatabaseBackendSettingsByBackendSettingsId.replaceAll(
                  "__typename",
                  ""
                ),
              variables: {
          ...
          ).data.backendDatabaseBackendSettingsByBackendSettingsId.items.map(
            (t) => t.backendDatabase
          )
        : [];
      setLinkedDatabases(linkedDatabases);
      const linkedQueues = record
        ? (
            await client.graphql({
              query: backendQueueBackendSettingsByBackendSettingsId.replaceAll(
                "__typename",
                ""
              ),
              variables: {
          ...
          ).data.backendQueueBackendSettingsByBackendSettingsId.items.map(
            (t) => t.backendQueue
          )
        : [];
      setLinkedQueues(linkedQueues);
      const linkedTopics = record
        ? (
            await client.graphql({
              query: backendTopicBackendSettingsByBackendSettingsId.replaceAll(
                "__typename",
                ""
              ),
              variables: {
          ...
          ).data.backendTopicBackendSettingsByBackendSettingsId.items.map(
            (t) => t.backendTopic
          )
        : [];
      setLinkedTopics(linkedTopics);
      setBackendSettingsRecord(record);
    };
    queryData();
  }, [idProp, backendSettingsModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const endpointsToLinkMap = new Map();
          const endpointsToUnLinkMap = new Map();
          const endpointsMap = new Map();
          const linkedEndpointsMap = new Map();
          endpoints.forEach((r) => {
            const count = endpointsMap.get(getIDValue.endpoints?.(r));
            const newCount = count ? count + 1 : 1;
            endpointsMap.set(getIDValue.endpoints?.(r), newCount);
          });
          linkedEndpoints.forEach((r) => {
            const count = linkedEndpointsMap.get(getIDValue.endpoints?.(r));
            const newCount = count ? count + 1 : 1;
            linkedEndpointsMap.set(getIDValue.endpoints?.(r), newCount);
          });
          linkedEndpointsMap.forEach((count, id) => {
            const newCount = endpointsMap.get(id);
            if (newCount) {
              const diffCount = count - newCount;
              if (diffCount > 0) {
                endpointsToUnLinkMap.set(id, diffCount);
              }
            } else {
              endpointsToUnLinkMap.set(id, count);
            }
          });
          endpointsMap.forEach((count, id) => {
            const originalCount = linkedEndpointsMap.get(id);
            if (originalCount) {
              const diffCount = count - originalCount;
              if (diffCount > 0) {
                endpointsToLinkMap.set(id, diffCount);
              }
            } else {
              endpointsToLinkMap.set(id, count);
            }
          });
          endpointsToUnLinkMap.forEach(async (count, id) => {
                ...
                variables: {
                  filter: {
                    and: [
                      ...
                      { backendSettingsId: { eq: backendSettingsRecord.id } },
                    ],
                  },
                },
              })
          ...
          endpointsToLinkMap.forEach((count, id) => {
            const backendEndpointToLink = endpointsRecords.find((r) =>
            ...
            for (let i = count; i > 0; i--) {
              promises.push(
                client.graphql({
                  ...
                  variables: {
                    input: {
                      backendSettingsId: backendSettingsRecord.id,
          ...
          const databasesToLinkMap = new Map();
          const databasesToUnLinkMap = new Map();
          const databasesMap = new Map();
          const linkedDatabasesMap = new Map();
          databases.forEach((r) => {
            const count = databasesMap.get(getIDValue.databases?.(r));
            const newCount = count ? count + 1 : 1;
            databasesMap.set(getIDValue.databases?.(r), newCount);
          });
          linkedDatabases.forEach((r) => {
            const count = linkedDatabasesMap.get(getIDValue.databases?.(r));
            const newCount = count ? count + 1 : 1;
            linkedDatabasesMap.set(getIDValue.databases?.(r), newCount);
          });
          linkedDatabasesMap.forEach((count, id) => {
            const newCount = databasesMap.get(id);
            if (newCount) {
              const diffCount = count - newCount;
              if (diffCount > 0) {
                databasesToUnLinkMap.set(id, diffCount);
              }
            } else {
              databasesToUnLinkMap.set(id, count);
            }
          });
          databasesMap.forEach((count, id) => {
            const originalCount = linkedDatabasesMap.get(id);
            if (originalCount) {
              const diffCount = count - originalCount;
              if (diffCount > 0) {
                databasesToLinkMap.set(id, diffCount);
              }
            } else {
              databasesToLinkMap.set(id, count);
            }
          });
          databasesToUnLinkMap.forEach(async (count, id) => {
            const recordKeys = JSON.parse(id);
            const backendDatabaseBackendSettingsRecords = (
              await client.graphql({
                query: listBackendDatabaseBackendSettings.replaceAll(
                  "__typename",
                  ""
                ),
                variables: {
                  filter: {
                    and: [
                      { backendDatabaseId: { eq: recordKeys.id } },
                      { backendSettingsId: { eq: backendSettingsRecord.id } },
                    ],
                  },
                },
              })
            )?.data?.listBackendDatabaseBackendSettings?.items;
            for (let i = 0; i < count; i++) {
              promises.push(
                client.graphql({
                  query: deleteBackendDatabaseBackendSettings.replaceAll(
                    "__typename",
                    ""
                  ),
                  variables: {
                    input: {
                      id: backendDatabaseBackendSettingsRecords[i].id,
                    },
                  },
                })
              );
          ...
          databasesToLinkMap.forEach((count, id) => {
            const backendDatabaseToLink = databasesRecords.find((r) =>
              Object.entries(JSON.parse(id)).every(
                ([key, value]) => r[key] === value
              )
            );
              ...
              promises.push(
                client.graphql({
                  ...
                  variables: {
                    input: {
                      backendSettingsId: backendSettingsRecord.id,
                      backendDatabaseId: backendDatabaseToLink.id,
                    },
                  },
                })
              );
          ...
          const queuesToLinkMap = new Map();
          const queuesToUnLinkMap = new Map();
          const queuesMap = new Map();
          const linkedQueuesMap = new Map();
          queues.forEach((r) => {
            const count = queuesMap.get(getIDValue.queues?.(r));
            const newCount = count ? count + 1 : 1;
            queuesMap.set(getIDValue.queues?.(r), newCount);
          });
          linkedQueues.forEach((r) => {
            const count = linkedQueuesMap.get(getIDValue.queues?.(r));
            const newCount = count ? count + 1 : 1;
            linkedQueuesMap.set(getIDValue.queues?.(r), newCount);
          });
          linkedQueuesMap.forEach((count, id) => {
            const newCount = queuesMap.get(id);
            if (newCount) {
              const diffCount = count - newCount;
              if (diffCount > 0) {
                queuesToUnLinkMap.set(id, diffCount);
              }
            } else {
              queuesToUnLinkMap.set(id, count);
            }
          });
          queuesMap.forEach((count, id) => {
            const originalCount = linkedQueuesMap.get(id);
            if (originalCount) {
              const diffCount = count - originalCount;
              if (diffCount > 0) {
                queuesToLinkMap.set(id, diffCount);
              }
            } else {
              queuesToLinkMap.set(id, count);
            }
          });
          queuesToUnLinkMap.forEach(async (count, id) => {
            const recordKeys = JSON.parse(id);
            const backendQueueBackendSettingsRecords = (
              await client.graphql({
                query: listBackendQueueBackendSettings.replaceAll(
                  "__typename",
                  ""
                ),
                variables: {
                  filter: {
                    and: [
                      { backendQueueId: { eq: recordKeys.id } },
                      { backendSettingsId: { eq: backendSettingsRecord.id } },
                    ],
                  },
                },
              })
            )?.data?.listBackendQueueBackendSettings?.items;
            for (let i = 0; i < count; i++) {
              promises.push(
                client.graphql({
                  query: deleteBackendQueueBackendSettings.replaceAll(
                    "__typename",
                    ""
                  ),
                  variables: {
                    input: {
                      id: backendQueueBackendSettingsRecords[i].id,
                    },
                  },
                })
              );
          ...
          queuesToLinkMap.forEach((count, id) => {
            const backendQueueToLink = queuesRecords.find((r) =>
              Object.entries(JSON.parse(id)).every(
                ([key, value]) => r[key] === value
              )
            );
              ...
              promises.push(
                client.graphql({
                  ...
                  variables: {
                    input: {
                      backendSettingsId: backendSettingsRecord.id,
                      backendQueueId: backendQueueToLink.id,
                    },
                  },
                })
              );
          ...
          const topicsToLinkMap = new Map();
          const topicsToUnLinkMap = new Map();
          const topicsMap = new Map();
          const linkedTopicsMap = new Map();
          topics.forEach((r) => {
            const count = topicsMap.get(getIDValue.topics?.(r));
            const newCount = count ? count + 1 : 1;
            topicsMap.set(getIDValue.topics?.(r), newCount);
          });
          linkedTopics.forEach((r) => {
            const count = linkedTopicsMap.get(getIDValue.topics?.(r));
            const newCount = count ? count + 1 : 1;
            linkedTopicsMap.set(getIDValue.topics?.(r), newCount);
          });
          linkedTopicsMap.forEach((count, id) => {
            const newCount = topicsMap.get(id);
            if (newCount) {
              const diffCount = count - newCount;
              if (diffCount > 0) {
                topicsToUnLinkMap.set(id, diffCount);
              }
            } else {
              topicsToUnLinkMap.set(id, count);
            }
          });
          topicsMap.forEach((count, id) => {
            const originalCount = linkedTopicsMap.get(id);
            if (originalCount) {
              const diffCount = count - originalCount;
              if (diffCount > 0) {
                topicsToLinkMap.set(id, diffCount);
              }
            } else {
              topicsToLinkMap.set(id, count);
            }
          });
          topicsToUnLinkMap.forEach(async (count, id) => {
            const recordKeys = JSON.parse(id);
            const backendTopicBackendSettingsRecords = (
              await client.graphql({
                ...
                variables: {
                  filter: {
                    and: [
                      { backendTopicId: { eq: recordKeys.id } },
                      { backendSettingsId: { eq: backendSettingsRecord.id } },
                    ],
                  },
                },
          ...
          topicsToLinkMap.forEach((count, id) => {
            const backendTopicToLink = topicsRecords.find((r) =>
              Object.entries(JSON.parse(id)).every(
                ([key, value]) => r[key] === value
              )
            );
              ...
              promises.push(
                client.graphql({
                  ...
                  variables: {
                    input: {
                      backendSettingsId: backendSettingsRecord.id,
                      backendTopicId: backendTopicToLink.id,
                    },
                  },
                })
              );
          ...
          promises.push(
            client.graphql({
              query: updateBackendSettings.replaceAll("__typename", ""),
              variables: {
                input: {
                  id: backendSettingsRecord.id,
                  ...modelFieldsToSave,
                },
              },
            })
      ...
      {...getOverrideProps(overrides, "BackendSettingsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || backendSettingsModelProp)}
