...
export default function AiProcessUpdateForm(props) {
  const {
    id: idProp,
    aiProcess: aiProcessModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = aiProcessRecord
      ? {
          ...initialValues,
          ...aiProcessRecord,
          taskType,
      ...
      : initialValues;
    setTebeDeleted(cleanValues.tebeDeleted);
    setName(cleanValues.name);
    setTaskType(cleanValues.taskType);
    setCurrentTaskTypeValue(undefined);
    setCurrentTaskTypeDisplayValue("");
    setAiConfig(cleanValues.aiConfig);
    ...
    setCommunicationSourceType(cleanValues.communicationSourceType);
    setFrontedWebsocketCommunicationSource(
      cleanValues.frontedWebsocketCommunicationSource
    );
    ...
    setBackendQueueCommunicationSource(
      cleanValues.backendQueueCommunicationSource
    );
    setCurrentBackendQueueCommunicationSourceValue(undefined);
    setCurrentBackendQueueCommunicationSourceDisplayValue("");
    setErrors({});
  };
  const [aiProcessRecord, setAiProcessRecord] =
    React.useState(aiProcessModelProp);
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getAiProcess.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAiProcess
        : aiProcessModelProp;
      const taskTypeRecord = record ? await record.taskType : undefined;
      setTaskType(taskTypeRecord);
      const aiConfigRecord = record ? await record.aiConfig : undefined;
      setAiConfig(aiConfigRecord);
      const linkedUsedIn = record
        ? (
            await client.graphql({
              query: aiProcessAiSettingsByAiProcessId.replaceAll(
                "__typename",
                ""
              ),
              variables: {
                aiProcessId: record.id,
              },
            })
          ).data.aiProcessAiSettingsByAiProcessId.items.map((t) => t.aiSettings)
        : [];
      setLinkedUsedIn(linkedUsedIn);
      const frontedWebsocketCommunicationSourceRecord = record
        ? await record.frontedWebsocketCommunicationSource
        : undefined;
      setFrontedWebsocketCommunicationSource(
        frontedWebsocketCommunicationSourceRecord
      );
      const backendQueueCommunicationSourceRecord = record
        ? await record.backendQueueCommunicationSource
        : undefined;
      setBackendQueueCommunicationSource(backendQueueCommunicationSourceRecord);
      setAiProcessRecord(record);
    };
    queryData();
  }, [idProp, aiProcessModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          communicationSourceType,
          frontedWebsocketCommunicationSource:
            frontedWebsocketCommunicationSource ?? null,
          backendQueueCommunicationSource:
            backendQueueCommunicationSource ?? null,
        };
        ...
        try {
            ...
            const aiProcessAiSettingsRecords = (
              await client.graphql({
                ...
                variables: {
                  filter: {
                    and: [
                      { aiSettingsId: { eq: recordKeys.id } },
                      { aiProcessId: { eq: aiProcessRecord.id } },
                    ],
                  },
                },
              })
          ...
          usedInToLinkMap.forEach((count, id) => {
            const aiSettingsToLink = usedInRecords.find((r) =>
            ...
            for (let i = count; i > 0; i--) {
              promises.push(
                client.graphql({
                  ...
                  variables: {
                    input: {
                      aiProcessId: aiProcessRecord.id,
                      aiSettingsId: aiSettingsToLink.id,
                    },
                  },
                })
              );
          ...
          const modelFieldsToSave = {
            ...
            taskTypeID: modelFields?.taskType?.id ?? null,
            aiConfigID: modelFields?.aiConfig?.id ?? null,
            communicationSourceType: modelFields.communicationSourceType,
            frontedWebsocketCommunicationSourceID:
              modelFields?.frontedWebsocketCommunicationSource?.id ?? null,
            backendQueueCommunicationSourceID:
              modelFields?.backendQueueCommunicationSource?.id ?? null,
          };
          promises.push(
            client.graphql({
              query: updateAiProcess.replaceAll("__typename", ""),
              variables: {
                input: {
                  id: aiProcessRecord.id,
                  ...modelFieldsToSave,
                },
              },
            })
      ...
      {...getOverrideProps(overrides, "AiProcessUpdateForm")}
      {...rest}
    >
      <SwitchField
        label="Tebe deleted"
      ...
      >
        <Autocomplete
          ...
          defaultValue={frontedWebsocketCommunicationSource}
          onChange={(e) => {
            let { value } = e.target;
      ...
      >
        <Autocomplete
          ...
          defaultValue={backendQueueCommunicationSource}
          onChange={(e) => {
            let { value } = e.target;
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || aiProcessModelProp)}
