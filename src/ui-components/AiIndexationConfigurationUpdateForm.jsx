...
import { updateAiIndexationConfiguration } from "../graphql/mutations";
...
export default function AiIndexationConfigurationUpdateForm(props) {
  const {
    id: idProp,
    aiIndexationConfiguration: aiIndexationConfigurationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = aiIndexationConfigurationRecord
      ? {
          ...initialValues,
          ...aiIndexationConfigurationRecord,
          vectorizerParameters,
          vectorialDbConfig,
        }
      : initialValues;
    setName(cleanValues.name);
    setVectorizerParameters(cleanValues.vectorizerParameters);
    setCurrentVectorizerParametersValue(undefined);
    setCurrentVectorizerParametersDisplayValue("");
    setVectorialDbConfig(cleanValues.vectorialDbConfig);
    setCurrentVectorialDbConfigValue(undefined);
    setCurrentVectorialDbConfigDisplayValue("");
    setErrors({});
  };
  const [aiIndexationConfigurationRecord, setAiIndexationConfigurationRecord] =
    React.useState(aiIndexationConfigurationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getAiIndexationConfiguration.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAiIndexationConfiguration
        : aiIndexationConfigurationModelProp;
      const vectorizerParametersRecord = record
        ? await record.vectorizerParameters
        : undefined;
      setVectorizerParameters(vectorizerParametersRecord);
      const vectorialDbConfigRecord = record
        ? await record.vectorialDbConfig
        : undefined;
      setVectorialDbConfig(vectorialDbConfigRecord);
      setAiIndexationConfigurationRecord(record);
    };
    queryData();
  }, [idProp, aiIndexationConfigurationModelProp]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          name,
          vectorizerParameters: vectorizerParameters ?? null,
          vectorialDbConfig: vectorialDbConfig ?? null,
        };
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            vectorizerParametersID:
              modelFields?.vectorizerParameters?.id ?? null,
            vectorialDbConfigID: modelFields?.vectorialDbConfig?.id ?? null,
          };
          await client.graphql({
            query: updateAiIndexationConfiguration.replaceAll("__typename", ""),
            variables: {
              input: {
                id: aiIndexationConfigurationRecord.id,
                ...modelFieldsToSave,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "AiIndexationConfigurationUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Autocomplete
          ...
          defaultValue={vectorizerParameters}
          onChange={(e) => {
            let { value } = e.target;
      ...
      >
        <Autocomplete
          ...
          defaultValue={vectorialDbConfig}
          onChange={(e) => {
            let { value } = e.target;
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || aiIndexationConfigurationModelProp)}
